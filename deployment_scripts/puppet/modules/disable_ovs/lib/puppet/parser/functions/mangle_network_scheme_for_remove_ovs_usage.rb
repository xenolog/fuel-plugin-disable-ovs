...
Puppet::Parser::Functions::newfunction( :mangle_network_scheme_for_remove_ovs_usage,
                                        :type => :rvalue, :doc => <<-EOS
    ...
    if argv.size != 1
      raise(
        Puppet::ParseError,
        "mangle_network_scheme_for_remove_ovs_usage(): Wrong number of arguments. Should be two."
      )
    end
    if !argv[0].is_a?(Hash)
      raise(
        Puppet::ParseError,
        "mangle_network_scheme_for_remove_ovs_usage(): Wrong network_scheme. Should be non-empty Hash."
      )
    end
    if argv[0]['version'].to_s.to_f < 1.1
      raise(
        Puppet::ParseError,
        "mangle_network_scheme_for_remove_ovs_usage(): You network_scheme hash has wrong format.\nThis parser can work with v1.1 format, please convert you config."
      )
    end
